...
export const MS: Record<string, MSRecord> = {
  ...
  // ── Q1: Thermoregulation ──
  q1a: { type: 'mcq', correct: 2 } as MCQEntry, // C — Musk ox

  q1b: {
    marks: 2,
    exemplar: 'Blubber (fat) is a poor conductor of heat [1]. It prevents / slows heat loss from the body to the cold water / surroundings [1].',
    keyConcepts: ['blubber is a poor conductor', 'insulating layer prevents heat loss', 'thermal insulation by fat'],
    keywords: ['blubber', 'fat', 'conductor', 'insulator', 'heat loss', 'prevent', 'slow', 'cold', 'water'],
    feedbackHit: 'Correctly explained how blubber acts as a thermal insulator to prevent heat loss.',
    feedbackMiss: 'Two ideas needed: (1) blubber (fat) is a poor conductor of heat, and (2) this insulating layer prevents / slows heat loss from the body to the surrounding cold water.',
  } as MSEntry,

  q1c: {
    marks: 2,
    exemplar: 'White fur AND reduces heat lost through (infra-red) radiation [1]. Large ears AND increases heat lost through conduction / convection / radiation (large surface area) [1].',
    ...
    feedbackMiss: 'Two separate ideas: (1) Arctic wolf — white fur reduces infra-red radiation emitted (helping in cold); (2) desert jerboa — large ears have large surface area which increases heat loss by conduction/convection/radiation (helping in heat).',
  } as MSEntry,

  q1d: {
    marks: 3,
    exemplar: 'Heat is required for evaporation OR evaporation removes heat from the skin [1]. High energy particles (in the water) escape from the surface [1]. Leaving lower energy particles behind / average kinetic energy of remaining particles decreases [1].',
    ...
    feedbackMiss: 'Three steps: (1) evaporation requires/takes heat energy from the water/skin, (2) the highest-energy (fastest-moving) particles escape from the surface, (3) remaining particles have lower average kinetic energy → the elephant cools down.',
  } as MSEntry,

  // ── Q2: La Marmotte / energy ──
  q2a: {
    marks: 2,
  ...
  q2b: {
    marks: 4,
    exemplar: 'Gain in height = 2600 − 800 = 1800 m [1]. ΔEp = mgh [1]. = 600 × 10 × 1800 = 10 800 000 J [1]. Correct unit: J or MJ [1].',
    keyConcepts: ['ΔEp = mgh', 'change in height = 1800 m', 'gravitational potential energy', 'unit: J or MJ'],
    keywords: ['GPE', 'gravitational', 'potential', 'mgh', '1800', 'height', '600', '10 800 000', 'joule', 'MJ'],
    feedbackHit: 'Correctly calculated the gain in gravitational potential energy including units.',
    feedbackMiss: 'Steps: (1) Δh = 2600 − 800 = 1800 m; (2) use ΔEp = mgh; (3) = 600 × 10 × 1800 = 10,800,000 J = 10.8 MJ. Unit must be consistent with the numerical answer.',
  } as MSEntry,

  q2c: {
    marks: 3,
    exemplar: 'Power = energy / time taken [1]. = 8 100 000 / 450 = 18 000 W [1]. Correct consistent unit: W or J s⁻¹ or kW [1].',
    keyConcepts: ['P = E / t', 'power in watts', 'unit consistency'],
    keywords: ['power', 'P = E/t', '8100000', '450', '18000', 'watts', 'W', 'kW'],
    feedbackHit: 'Correctly calculated power and included a consistent unit.',
    feedbackMiss: 'P = E / t = 8,100,000 J ÷ 450 s = 18,000 W = 18 kW. The unit must be consistent with your numerical answer.',
  } as MSEntry,
  ...
  q3c: {
    marks: 1,
    exemplar: 'Satellite phone communication OR weather forecasting OR television relay OR military intelligence (any one).',
    keyConcepts: ['geostationary satellite applications', 'weather forecasting', 'satellite phone', 'TV relay'],
    keywords: ['weather', 'phone', 'TV', 'television', 'relay', 'military', 'communication', 'satellite'],
    feedbackHit: 'Gave a valid application of geostationary satellites.',
    feedbackMiss: 'Other applications of geostationary satellites include: satellite phone communication, weather forecasting, TV/radio relay, and military intelligence. Do not repeat GPS which was mentioned in the question.',
  } as MSEntry,
  ...
  q4a: {
    marks: 1,
    exemplar: 'How does the current flowing through the coil affect the strength of the electromagnet / weight of metal staples supported?',
    keyConcepts: ['research question', 'current as independent variable', 'electromagnet strength as dependent variable'],
    keywords: ['current', 'affect', 'electromagnet', 'strength', 'staples', 'weight', 'research question'],
    feedbackHit: 'Correctly stated a research question with current as IV and electromagnet strength as DV.',
    feedbackMiss: 'The research question must name the independent variable (current) and what you are measuring (electromagnet strength / weight of staples). E.g. "How does the current affect the weight of metal staples supported by the electromagnet?"',
  } as MSEntry,

  q4b: {
    marks: 4,
    exemplar: 'Independent: Current only [1]. Dependent: Total weight of metal staples supported only [1]. Control variables: Diameter of iron rod AND Number of turns AND Total length AND Size of staples [1 for 2 correct; 2 for all 4 correct].',
    keyConcepts: ['current = independent variable', 'weight of staples = dependent variable', 'diameter/turns/length/size = control variables'],
    ...
    feedbackMiss: 'Independent = only Current (what you change). Dependent = only Total weight of metal staples (what you measure). Everything else (diameter of iron rod, number of turns, total length, size of staples) = Control (kept the same).',
  } as MSEntry,

  q4c: {
    marks: 3,
    exemplar: 'If: the current increases [1]. Then: the weight of metal staples supported will increase / the strength of the electromagnet will increase [1]. Because: the electromagnetic field is stronger [1].',
    ...
    feedbackMiss: 'Structure as If→Then→Because: If [current increases], then [weight of staples supported increases], because [greater current creates a stronger electromagnetic field]. All three parts needed for full marks.',
  } as MSEntry,

  q4d: {
    marks: 2,
    exemplar: 'Ammeter selected [1] to measure the size of the current [1]. OR balance/scale selected [1] to weigh the total weight of metal staples supported [1].',
    ...
    feedbackMiss: 'You need either: an ammeter to measure the current, OR a balance/scale to measure the weight of staples supported. State both the equipment name and its specific purpose.',
  } as MSEntry,
  ...
  q4e: {
    ...
    feedbackMiss: 'Select the balance that has the greatest measurement range. Justify: it needs to measure a range of weights from small to larger values as more staples are added.',
  } as MSEntry,
  ...
  q5a: {
    ...
    keywords: ['table', 'headings', 'units', 'length', 'staples', 'ascending', 'descending', 'cm'],
    feedbackHit: 'Correctly presented raw data in a properly formatted table.',
    feedbackMiss: 'Raw data table needs: (1) a header row with column names and units (e.g. Length of coil / cm; Number of staples), (2) all 6 data rows correct (1.0→6, 2.0→3, 3.0→2, 4.0→1, 5.0→1, 6.0→1), (3) data in order.',
  } as MSEntry,

  q5b: {
    marks: 4,
    exemplar: 'Processed table headers: Turns per centimetre / cm⁻¹ AND Weight of staples / N [1]. Correct turns/cm for all rows: 6.0, 3.0, 2.0, 1.5, 1.2, 1.0 [1]. Correct weights for all rows: 3.0, 1.5, 1.0, 0.5, 0.5, 0.5 N [1]. All values to 1 d.p. [1].',
    keyConcepts: ['turns per centimetre = turns ÷ length', 'weight = staples × 0.5 N', 'processed data table', 'correct units'],
    keywords: ['turns per cm', 'weight', 'N', '0.5', 'calculate', 'divide', 'multiply', 'table', 'cm⁻¹'],
    feedbackHit: 'Correctly calculated and presented all processed data values.',
    feedbackMiss: 'Calculate: turns/cm = number of turns ÷ length in cm. Weight = staples × 0.5 N. Values: 6.0 cm⁻¹ → 3.0 N; 3.0 → 1.5 N; 2.0 → 1.0 N; 1.5 → 0.5 N; 1.2 → 0.5 N; 1.0 → 0.5 N. All to 1 d.p.',
  } as MSEntry,

  q5c: {
    marks: 2,
    exemplar: 'This would lead to more accurate data [1]. Because small pins produce a more precise measure of weight supported (0.02 N increments vs 0.5 N increments for staples) [1].',
    keyConcepts: ['small increments give more precision', 'accurate data', '0.02 N vs 0.5 N'],
    keywords: ['accurate', 'precise', 'small', 'increment', '0.02', 'better', 'measurement', 'resolution'],
    feedbackHit: 'Correctly justified why small pins give better data.',
    feedbackMiss: 'Two marks: (1) the data will be more accurate/precise, (2) because each small pin adds only 0.02 N (vs 0.5 N per staple) — this gives a much finer measurement scale.',
  } as MSEntry,
  ...
  q6a: {
    marks: 2,
    exemplar: 'Height for tube C measured correctly: 7.0 ± 0.5 mm [1]. Table completed correctly with diameter 4.0 mm, 1/diameter 0.25 mm⁻¹, height 7.0 mm [1].',
    keyConcepts: ['measure height from diagram', 'complete data table', '1/diameter column'],
    keywords: ['measure', 'height', '7', 'diameter', '4.0', '0.25', 'table', 'complete'],
    feedbackHit: 'Correctly measured tube C height and completed the table.',
    feedbackMiss: 'Read the height of water in tube C from the diagram — approximately 7.0 mm. Then complete the row: diameter = 4.0 mm, 1/diameter = 1/4.0 = 0.25 mm⁻¹, height ≈ 7.0 mm.',
  } as MSEntry,

  q6b: {
    marks: 4,
    exemplar: 'Point C plotted correctly at (0.25, 7.0) [1]. Line of best fit drawn [1]. Approximately equal distribution of points above and below the line [1]. Both axes labelled with units: x = 1/diameter / mm⁻¹, y = Height of liquid / mm [1].',
    ...
    feedbackMiss: 'Plot C at (0.25, 7.0). Draw a straight line of best fit with roughly equal numbers of points above and below. Label x-axis: "1/diameter / mm⁻¹" and y-axis: "Height of liquid / mm".',
  } as MSEntry,

  q6c: {
    marks: 3,
    exemplar: 'Two points from line of best fit chosen with Δy ≥ 10 [1]. Gradient = Δy / Δx shown [1]. Final value ≈ 28 ± 5 (units not required, accept 23–33) [1].',
  ...
  q7a: {
    marks: 13,
    exemplar: 'Full experimental plan: Research question identifying detergent concentration (IV) and capillary height (DV). Variables: IV = detergent concentration (mL/100 mL water), DV = height of liquid rise (mm), CVs = tube diameter, liquid temperature, tube length. Equipment includes graduated pipette, ruler, beaker, capillary tube — each with stated purpose. Detailed method. At least 5 concentration increments AND 3 trials with plan to calculate mean.',
    keyConcepts: ['research question', 'IV = detergent concentration', 'DV = height', 'controlled variables', 'graduated pipette', 'capillary tube', 'ruler', 'method', 'five increments', 'three trials', 'mean'],
    keywords: ['detergent', 'concentration', 'height', 'IV', 'DV', 'control', 'pipette', 'capillary', 'ruler', 'method', 'repeat', 'mean', 'trials', 'increments'],
    feedbackHit: 'Provided a complete and detailed experimental design covering all required elements.',
    feedbackMiss: 'Include: (1) Research question with IV=detergent concentration, DV=height; (2) list IV, DV, and at least one CV (tube diameter); (3) list equipment with purpose; (4) detailed method; (5) at least 5 different concentrations and 3 trials per concentration to calculate mean.',
  } as MSEntry,

  // ── Q8: Maglev trains ──
  q8a: { type: 'mcq', correct: 2 } as MCQEntry, // Iron (index 2 in 0-based: Platinum=0, Tin=1, Iron=2, Gold=3)
