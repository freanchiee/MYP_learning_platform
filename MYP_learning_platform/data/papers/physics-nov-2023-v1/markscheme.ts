...
export const MS: Record<string, MSRecord> = {
  ...
  // Q2 — Iodine-131 medical tracer
  'q2_a': {
    marks: 2,
    exemplar: 'Number of protons = 53 (the atomic number). Number of neutrons = 131 − 53 = 78.',
    keyConcepts: ['protons equals atomic number', '53 protons', '78 neutrons', 'mass number minus atomic number'],
    keywords: ['53', '78', 'protons', 'neutrons', 'atomic number', 'mass number', 'subtract'],
    feedbackHit: 'Correct — protons = atomic number = 53; neutrons = mass number − atomic number = 78.',
    feedbackMiss: 'Protons = atomic number = 53. Neutrons = mass number − atomic number = 131 − 53 = 78.',
  },
  'q2_b': {
    marks: 2,
    exemplar: 'Type of decay: beta (β⁻) decay. One product (apart from xenon-131): a beta particle (fast-moving electron, β⁻) or an antineutrino.',
    ...
    feedbackMiss: 'I-131 → Xe-131 is beta (β⁻) decay. A neutron converts to a proton, emitting a beta particle (electron) and an antineutrino.',
  },
  'q2_c': {
    marks: 1,
    exemplar: 'Half-life of iodine-131 ≈ 8 days (accept 7.5–8.5 days from reading the graph where 100% → 50%).',
    keyConcepts: ['half life', '8 days', 'time for 50 percent', 'graph reading'],
    keywords: ['8', '7.5', '8.5', 'half', '50%', 'days'],
    feedbackHit: 'Correct graph reading — half-life approximately 8 days.',
    feedbackMiss: 'Find where the curve reaches 50% on the y-axis, then read the corresponding x-value. This gives the half-life ≈ 8 days.',
  },
  'q2_d': {
    marks: 1,
    exemplar: '25% remaining means the sample has gone through 2 half-lives (100% → 50% → 25%). From the graph, at 25% the time is approximately 16 days (2 × 8 days).',
    keyConcepts: ['25 percent', '2 half-lives', '16 days', 'graph reading'],
    keywords: ['25%', '16', '2 half-lives', 'days', 'two'],
    feedbackHit: 'Correct — reading ~16 days from the graph at 25% (two half-lives).',
    feedbackMiss: 'Read the time on the x-axis where the curve passes through y = 25%. 25% is two half-lives (100→50→25), so ≈ 16 days.',
  },
  'q2_e': {
    marks: 2,
    exemplar: 'The claim is incorrect/invalid. The half-life of iodine-131 is only about 8 days. After a few thousand years the amount of iodine-131 remaining would be so tiny (effectively zero) that it could not be detected or measured. The graph shows that iodine-131 is essentially gone within about 40–50 days. Because its half-life is far too short, iodine-131 cannot be used to date objects that are thousands of years old — a long-lived isotope such as carbon-14 (half-life ~5,700 years) is needed instead.',
    keyConcepts: ['incorrect claim', 'half-life too short', 'undetectable', 'gone within weeks', 'cannot date thousands of years'],
    keywords: ['incorrect', 'invalid', 'too short', 'zero', 'undetectable', '8 days', '40', '50', 'cannot', 'impossible'],
    feedbackHit: 'Excellent evaluation — the claim correctly rejected using evidence from the graph.',
    feedbackMiss: 'I-131 has a half-life of ~8 days. Within ~40–50 days essentially none remains, so it cannot date objects thousands of years old. A long-lived isotope (e.g. carbon-14) is required for that.',
  },

  // Q3 — Halogen vs LED Spotlight
  'q3_a': {
    marks: 2,
  ...
  'q3_c': {
    marks: 2,
    exemplar: 'P = IV = 0.04 A × 12 V = 0.48 W',
    keyConcepts: ['P equals IV', '0.48 watts', 'power formula', 'substitution'],
    keywords: ['p=iv', '0.48', 'watts', 'power', 'multiply', '12', '0.04'],
    feedbackHit: 'Correct calculation: P = IV = 0.04 × 12 = 0.48 W.',
    feedbackMiss: 'Use P = IV. Substitute I = 0.04 A and V = 12 V: P = 0.04 × 12 = 0.48 W.',
  },
  'q3_d': {
    marks: 3,
    exemplar: 'Halogen efficiency = useful output / total input = 5/100 = 0.05 (5%). LED efficiency = 0.20 W / 0.48 W = 0.42 (42%). The LED is much more efficient (42% vs 5%) — it converts a much greater proportion of the electrical energy into useful light energy and wastes far less as heat.',
    keyConcepts: ['efficiency formula', '5 percent halogen', '42 percent LED', 'LED more efficient', 'comparison'],
    keywords: ['efficiency', '5%', '42%', 'more efficient', '0.05', '0.42', 'halogen', 'LED', 'useful', 'output/input'],
    feedbackHit: 'Both efficiencies calculated correctly and a clear comparison made.',
    feedbackMiss: 'Efficiency = useful power out / total power in. Halogen: 5/100 = 5%. LED: 0.20/0.48 = 42%. The LED is far more efficient.',
  },

  // Q4 — Basketball Bounce
  'q4_a': {
    marks: 1,
    exemplar: 'How does the drop height of a basketball affect the height of the first bounce on a wooden gym floor?',
  ...
  // Q5 — Coefficient of Restitution (Squash Ball)
  'q5_a': {
    marks: 1,
  ...
  'q5_b': {
    marks: 1,
    exemplar: 'If e = 0, the squash ball would not bounce at all — it would come to rest immediately upon hitting the ground (all kinetic energy is lost in the collision).',
  ...
  'q5_e': {
    marks: 3,
    exemplar: 'The statement is incorrect. If e = 0.6, then v₂ = 0.6v₁. The fraction of KE remaining = KE₂/KE₁ = (½mv₂²)/(½mv₁²) = (v₂/v₁)² = e² = (0.6)² = 0.36. So 36% of the KE remains after the collision, meaning 64% is converted — not 60% as the student claimed.',
    keyConcepts: ['e squared', '0.36', '36 percent remains', '64 percent converted', 'incorrect statement'],
    keywords: ['e squared', '0.36', '36%', '64%', 'incorrect', 'KE', 'kinetic', 'v squared', '0.6'],
    feedbackHit: 'Excellent — correctly derived that KE fraction = e², so e = 0.6 means 64% KE converted, not 60%.',
    feedbackMiss: 'KE ∝ v². If e = v₂/v₁ = 0.6, then KE₂/KE₁ = (v₂/v₁)² = e² = 0.36. Only 36% KE remains, so 64% is converted. The statement is wrong.',
  },

  // Q6 — Golf-Ball Coating Investigation
  'q6_a': {
    marks: 14,
    exemplar: 'Independent variable: thickness of soft cover layer (varied from a thin coating to 2.5 mm)\nDependent variable: height of first bounce (measured with ruler)\nControl variable (with justification): drop height — keep constant (e.g. 1.0 m) to ensure any change in bounce height is due to cover thickness only\n\nHypothesis: As cover thickness increases, the height of the first bounce will decrease, because a thicker soft cover absorbs more energy as heat during the collision (it deforms more), so less kinetic energy is returned to the ball.\n\nEquipment: Golf balls with different cover thicknesses (e.g. 0.5, 1.0, 1.5, 2.0, 2.5 mm), hard steel plate, metre ruler, clamp stand, retort stand, camera (optional for accuracy)\n\nMethod: 1. Set up ruler vertically beside the steel plate. 2. Drop a ball from a fixed height (e.g. 1.0 m) onto the plate. 3. Measure the height of the first bounce using the ruler. 4. Repeat 3 times for each cover thickness. 5. Calculate the average. 6. Repeat for each cover thickness (0.5–2.5 mm in 0.5 mm steps).',
    ...
    keywords: ['cover', 'thickness', 'bounce', 'height', 'ruler', 'control', 'drop height', 'repeat', 'average', 'hypothesis', 'equipment'],
    feedbackHit: 'Comprehensive investigation plan with all required components.',
    feedbackMiss: 'Check: Did you state IV (cover thickness), DV (bounce height), a justified CV, a hypothesis with explanation, equipment, and a step-by-step method with repeats?',
  },
  'q6_b': {
    marks: 4,
  ...
  // Q7 — Super-Insulated Cabin
  'q7_a': {
    marks: 2,
    exemplar: 'Super-insulated earth-sheltered cabins use thick insulation in walls and roof to reduce heat loss by conduction. Airtight construction prevents heat loss by convection (drafts). Triple-glazed windows reduce heat loss and allow solar gain, and the surrounding earth stays at a stable temperature so less heat is lost through the buried walls. These features greatly reduce the rate of heat transfer from inside to outside, so less energy input is needed to maintain a warm temperature.',
    keyConcepts: ['insulation', 'reduces conduction', 'airtight', 'reduces convection', 'earth covering stable temperature', 'less energy needed'],
    keywords: ['insulation', 'conduction', 'convection', 'airtight', 'earth', 'soil', 'heat loss', 'reduce', 'less energy', 'thermal'],
    feedbackHit: 'Good explanation linking the cabin features to reduced heat loss.',
    feedbackMiss: 'These cabins reduce heat loss by: insulation (less conduction), airtightness (less convection/drafts), triple glazing, and earth covering (stable surrounding temperature). Less heat escapes → less energy needed.',
  },
  'q7_b': {
    marks: 2,
  ...
  'q7_d': {
    marks: 1,
    exemplar: 'Approximately 15% of households use wood/biomass as a source of energy for heating (reading from the stacked bar chart).',
    keyConcepts: ['15 percent', 'wood', 'biomass', 'bar chart reading'],
    keywords: ['15', 'percent', '%', 'wood', 'biomass'],
    feedbackHit: 'Correct reading from the bar chart.',
    feedbackMiss: 'Read the wood/biomass section from the stacked bar chart. It occupies the band from about 75–90%, making it about 15%.',
  },
  'q7_e': {
    marks: 2,
    exemplar: 'Super-insulated housing reduces energy consumption for heating. This means less fossil fuel is burned, leading to lower CO₂ and other greenhouse gas emissions. This reduces the contribution to global warming/climate change, making it environmentally beneficial. Less fuel use also reduces air pollution.',
  ...
  // Q8 — Insulated Housing Policy
  'q8_': {
    marks: 13,
    exemplar: 'Social/economic positives for individuals: reduced heating bills (saves ~€760/year), improved health (better air quality, warmer home reduces cold-related illness), reduced fuel poverty. Negatives: high upfront retrofit cost (€18,000), disruption during renovation, low-income households may not afford the upgrade even with a subsidy.\n\nFor governments/businesses: Positive: reduced national energy consumption, meets climate targets, creates jobs in construction/retrofit sector. Negative: high public expenditure (many houses to upgrade), slow regulatory process, may increase inequality if only wealthier households can afford it.\n\nConclusion: Overall, government funding is justified because the long-term savings (energy, healthcare, environmental) outweigh the costs, and it addresses fuel poverty by helping those who cannot afford the upgrade independently.',
