...
export const MS: MSRecord = {
  ...
  // Q1 — Ankle joint, muscles & respiration

  q1_a: {
    marks: 1,
    exemplar: 'Elbow. (Accept knee or knuckle.)',
    keyConcepts: ['hinge joint skeleton movement bones cartilage'],
    keywords: ['hinge joint', 'elbow', 'knee', 'knuckle'],
    feedbackHit: 'Correct hinge joint identified.',
    feedbackMiss: 'Hinge joints move in one plane: elbow, knee, and knuckle are all valid examples.',
  },
  ...
  q1_c: {
    marks: 2,
    exemplar: 'Accept any two from: one muscle contracts while the other extends [1]; X (Gastrocnemius/calf) contracts or shortens AND to point the foot down or plantar flex [1]; Y (Tibialis anterior) contracts or shortens AND to pull the foot up or dorsiflex [1].',
    keyConcepts: ['antagonistic muscles gastrocnemius tibialis contract extend movement joint'],
    keywords: ['contracts', 'extends', 'gastrocnemius', 'tibialis', 'plantar flex', 'dorsiflex', 'point down', 'antagonistic'],
    feedbackHit: 'Both antagonistic muscle roles correctly described.',
    feedbackMiss: 'Gastrocnemius (calf) contracts to point the foot down; tibialis anterior contracts to pull it up. They work as an antagonistic pair — when one contracts the other extends.',
  },
  ...
  // Q2 — Edible coatings & food storage

  q2_a: {
    marks: 2,
    exemplar: 'Accept any two reasonable benefits: slows spoilage or oxidation [1]; prevents microbe growth OR keeps food safe to eat for longer [1]; reduces waste OR extends shelf life [1]. (WTTE)',
    keyConcepts: ['edible coating food preservation spoilage oxygen microbes shelf life transport'],
    keywords: ['slows spoilage', 'prevents microbe growth', 'extends shelf life', 'reduces waste', 'food safe'],
    feedbackHit: 'Two valid benefits of an edible coating identified.',
    feedbackMiss: 'An edible coating: slows spoilage (keeps oxygen out), prevents microbial growth, extends shelf life, reduces food waste.',
  },
  ...
  q2_c: {
    ...
    keyConcepts: ['osmosis bacteria sugar dehydration water loss shrinkage preservation'],
    keywords: ['shrunk', 'shrivelled', 'dehydrated', 'water left', 'osmosis', 'sugar', 'concentration gradient'],
    feedbackHit: 'Correct osmosis explanation with dehydration and shape change.',
    feedbackMiss: 'The high sugar concentration in jam creates a high solute concentration outside the bacteria. By osmosis, water moves out of the bacteria → they dehydrate and shrink, losing their smooth surface.',
  },

  // Q3 — Enzymes, amylase & genetics
  ...
  q3_b: {
    marks: 1,
    exemplar: 'AA = Yes; Aa = Yes; aa = No. All correct = 1 mark. (A selection must be made in ALL boxes.)',
    keyConcepts: ['genotype phenotype dominant recessive allele amylase production AA Aa aa'],
    keywords: ['AA', 'Aa', 'aa', 'dominant', 'recessive', 'amylase', 'Yes', 'No'],
    feedbackHit: 'All three genotype-phenotype relationships correctly identified.',
    feedbackMiss: 'A is dominant: AA (homozygous dominant) = high amylase; Aa (heterozygous) = high amylase (A present); aa (homozygous recessive) = low.',
  ...
  q3_d: {
    marks: 4,
    exemplar: 'Similarity [max 1]: highest levels directly after birth; both decrease (after birth). Justification [max 1]: maternal milk is the primary food source early on; decrease as they transition to other food sources. Difference [max 1]: production is much higher in foals; foals level off later than rabbits; production decreases at a faster rate or earlier in rabbits. Justification [max 1]: foals consume more starch OR foals are bigger than rabbits; rabbits transition to other foods earlier than foals; foals and rabbits mature at different rates.',
    keyConcepts: ['amylase production foals rabbits comparison similarity difference graph analysis justification'],
    keywords: ['highest at birth', 'both decrease', 'foals higher', 'foals level off later', 'maternal milk', 'transition to food'],
    feedbackHit: 'Similarity + justification + difference + justification all correctly identified.',
    feedbackMiss: 'Similarity: both start high and decrease. Difference: amylase is much higher in foals and stays elevated longer (foals are larger/take longer to wean). Justify each with scientific reasoning.',
  },

  q3_e: {
    marks: 1,
    exemplar: 'The higher the % of adults with high amylase, the higher the mean starch-rich plant intake. (ORA. Do NOT accept linear or proportional.)',
    keyConcepts: ['amylase production starch intake correlation population data infographic'],
    keywords: ['higher amylase', 'higher intake', 'positive relationship', 'correlation'],
    feedbackHit: 'Correct directional relationship stated.',
    feedbackMiss: 'Populations with a higher % of adults producing high amylase tend to have higher mean starch-rich plant intake (positive correlation).',
  },

  q3_f: {
    marks: 1,
    exemplar: 'Accept any reasonable suggestion: data not available for all populations; correlation does not guarantee causation; people eat starch-rich plants for reasons other than amylase production; data is from one year or one source only.',
    keyConcepts: ['scientific caution correlation causation data limitations population data'],
    ...
    feedbackMiss: 'Reasons for caution: data missing for many populations; correlation ≠ causation; other factors affect intake; data from a single time point.',
  },

  // Q4 — Jute Fibre Humidity Investigation

  q4_a: {
    marks: 1,
    exemplar: 'How does the humidity affect the mass needed to break the fibre? (Must link humidity to mass/strength to be credited.)',
    keyConcepts: ['research question humidity jute plant fibre strength investigation formulate'],
    ...
    feedbackMiss: 'Research question must state BOTH the IV (humidity) and DV (mass/force needed to break fibre), e.g. "How does humidity affect the mass needed to break jute fibre?"',
  },
  ...
  q4_f: {
    marks: 4,
    exemplar: 'Equal increments on Y axis scale [1]; three points plotted correctly [1]; all eight points plotted correctly [1]; data points matching the key [1]. (Ignore the key for first 2 marks; only award mark 4 if at least 3 points correctly plotted.) Group 1 (20,70)(40,120)(60,155)(80,170); Group 2 (20,55)(40,100)(60,150)(80,200).',
    keyConcepts: ['graph plotting data presentation humidity mass jute fibre two groups key'],
    keywords: ['equal increments', 'Y axis', 'plotted correctly', 'key', 'Group 1', 'Group 2'],
    feedbackHit: 'Graph has equal Y-scale, all 8 points plotted correctly with key.',
    feedbackMiss: 'Graph needs: equal Y-axis increments; all 8 data points correctly plotted; a key differentiating Group 1 and Group 2.',
  },
  ...
  q4_h: {
    ...
    feedbackMiss: 'Claim 1 (different strengths): natural variation in jute fibres (fibres from different plants vary). Claim 2 (different masses): less precise measuring equipment produced less valid/accurate results.',
  },

  // Q5 — Jute Cell Diameter & Water Movement

  q5_a: {
    marks: 1,
    exemplar: '91 (micrometres) ± 5. (Ignore incorrect units.)',
    keyConcepts: ['measuring cell diameter microscope scale bar micrometer humidity'],
    keywords: ['91', 'micrometres', 'diameter', 'cell', 'scale bar', 'measure'],
    feedbackHit: 'Cell diameter correctly measured from scale bar.',
    feedbackMiss: 'Use the 70 µm scale bar to calibrate. The highlighted cell at 75% humidity measures approximately 91 µm in diameter (± 5 µm accepted).',
  },

  q5_b: {
    marks: 3,
    exemplar: '(Candidate\'s value from (a) − 70) ÷ 70 × 100 [1]. Evidence of calculation [1]. Rounded to nearest % [1]. (ECF from part (a). Award third mark for any correctly rounded percentage if calculation is seen.) Using 91: (91 − 70) ÷ 70 × 100 = 30%.',
    keyConcepts: ['percentage increase calculation diameter cell humidity formula ECF'],
    keywords: ['% increase', '70', 'difference', 'divide', 'multiply by 100', 'nearest %'],
    feedbackHit: 'Correct formula applied, calculation shown, answer rounded appropriately.',
    feedbackMiss: '% increase = (new − original) ÷ original × 100 = (91 − 70) ÷ 70 × 100 = 30%. Show working.',
  },
  ...
  q6_a: {
    marks: 1,
    exemplar: 'Accept any CV from: volume of NaOH (40 cm³); fibres immersed in same chemical or NaOH; initial fibre length (25 cm) or diameter; type of fibre (jute).',
    ...
    feedbackMiss: 'The flow chart shows CVs such as: volume of NaOH used (40 cm³), initial fibre length (25 cm)/diameter, type of plant fibre (jute). All fibres are treated the same to control these.',
  },
  ...
  q6_c: {
    marks: 3,
    exemplar: 'Accept any three points: identify the alkali (as NaOH) [1]; include a reference to the direction of change of IV [1]; include a possible range of IV concentrations [1]; specify fibres are jute [1]; specify what will be measured [1]. (Accept aspects of a re-written hypothesis. Do NOT accept "include adding scientific justification as an improvement.")',
    keyConcepts: ['hypothesis testable IV direction range NaOH concentration alkali jute fibre measurement'],
    keywords: ['NaOH', 'concentration increases', 'range', 'jute', 'extension', 'measured', 'testable'],
    feedbackHit: 'Three elements to improve hypothesis correctly identified.',
    feedbackMiss: 'Improve hypothesis by: naming NaOH as the alkali; specifying direction (as concentration increases); including a range (e.g. 0–10%); specifying it is jute fibre; stating what is measured (extension in mm).',
  },

  q6_d: {
    marks: 2,
    exemplar: 'First marking point: hypothesis is valid or partially valid [1]; as the extension increases from 0–2% or 5% (NaOH) [1]. Second marking point linked to hypothesis being invalid [1]: between 5–10% the extension decreases; above 10% there is no clear increase or decrease despite the change in concentration. (Accept reference to a plateau. Do NOT accept constant.)',
    keyConcepts: ['graph results hypothesis support partial valid extension NaOH concentration'],
    keywords: ['partially valid', 'increases 0–5%', 'decreases 5–10%', 'plateau above 10%', 'partially supported'],
    feedbackHit: 'Correct evaluation of hypothesis against graph data at both concentration ranges.',
    feedbackMiss: '0–5% NaOH: extension increases → hypothesis supported. Above 5–10%: extension decreases/plateaus → hypothesis not supported. Result: partially valid.',
  },

  // Q7 — Criteria B: Jute Fibre % in Fabric Design

  q7_: {
    marks: 15,
    exemplar: 'Criteria B rubric (max 15). RQ [max 2]: simple RQ → RQ linked to % jute fibre and length/stretch. Variables [max 4]: IV as fibre % and DV as length + one CV → IV and DV + two CVs. Equipment [assessed in Method]. Method [max 4]: attempt at method → incomplete but some relevant data → sufficient to follow and collect similar data → sufficient to repeat. Data [max 4]: plans at least 3 trials → plans 3 trials AND 5 stated fibre % → also includes 0% and 20% → 5+ fibre %. Safety [max 1]: care with heavy masses, fabric tearing under tension.',
    keyConcepts: ['jute fibre fabric stretch design investigation Criteria B RQ IV DV CVs method data safety'],
    keywords: ['% jute fibres', 'IV', 'DV', 'extension/stretch', 'CVs', 'repeats', '5 fibre levels', '0%', '20%', 'safety'],
    ...
    feedbackMiss: 'Key elements: RQ linking % jute fibres to stretch/extension; IV = % jute fibres; DV = extension/length at fixed mass; 2 CVs (e.g. mass, fabric width); ≥5 fibre % values including 0% and 20%; ≥3 repeats; safety (heavy masses, fabric tearing).',
  },
  ...
  q8_b: {
    marks: 4,
    exemplar: 'Fewer moths will mean there is less pollination of the flowers [1]. This may reduce the population of plants [1]. (Which will) result in less food being available (for consumers) [1]. Link to how biodiversity is reduced [1]. (Accept references to plant reproduction.)',
    keyConcepts: ['pollinators moths pollination biodiversity food web plants flowers consumers'],
    keywords: ['fewer moths', 'less pollination', 'plant population reduces', 'less food', 'biodiversity reduced'],
    feedbackHit: 'Complete causal chain from pollinator loss to biodiversity reduction.',
    feedbackMiss: 'Fewer moths → less pollination → fewer plants reproduce → plant population falls → less food for consumers → animals affected → biodiversity reduced.',
  },
  ...
  // Q9 — Aquavoltaic Biosolar Technology (Criteria D)

  q9_: {
    marks: 13,
    exemplar: 'Criteria D rubric (max 13). Environmental [max 4]: one aspect → one wetland OR solar panel + one other → both with further justification → both aspects with further justification for both. Economic [max 4]: same progression. Aquavoltaic additional benefits [max 2]: one benefit (solar panels work more efficiently when cooled by the water beneath / floating panels reduce evaporation from the pond / no land is used) → with further development. Location factor [max 2]: one reasonable factor (e.g. unshaded water surface, water depth, access for maintenance) → with justification. Conclusion [max 1]: a concluding statement.',
    keyConcepts: ['aquavoltaic biosolar technology constructed wetland floating solar panels environmental economic sustainability urban biodiversity'],
    keywords: ['constructed wetland', 'floating solar panels', 'environmental', 'economic', 'biosolar', 'location', 'cooling', 'biodiversity', 'conclusion'],
    feedbackHit: 'Comprehensive Criteria D covering environmental, economic, combined aquavoltaic benefits, location factor, and conclusion.',
    feedbackMiss: 'Environmental: constructed wetlands (biodiversity, water filtering, habitat, flood storage); solar panels (renewable energy, reduce fossil fuels). Economic: installation cost vs energy savings. Aquavoltaic: panels cooled by water work better; floating panels cut evaporation and use no land. Location: consider unshaded water, depth, maintenance access. Conclude on overall impact.',
  },
}
