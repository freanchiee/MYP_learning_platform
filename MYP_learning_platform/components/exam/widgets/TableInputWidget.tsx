          ...
          {parsedRows.map((cells, ri) => (
            <tr key={ri} className={ri % 2 === 0 ? 'bg-white' : 'bg-gray-50/60'}>
              {cells.map((cell, ci) => {
                ...
                if (cell === '~') {
                  return (
                    <td key={ci} className="px-2 py-1.5 border-t border-gray-100">
                      <input
                        type="text"
                        value={values[key] ?? ''}
                        onChange={e => handleChange(key, e.target.value)}
                        className="w-full rounded-lg border border-gray-300 px-2.5 py-1.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[var(--ib-teal)] focus:border-transparent transition"
                        placeholder="…"
