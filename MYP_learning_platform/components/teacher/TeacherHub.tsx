...
const btnSolid = { background: '#fff', color: '#160f2a' } as const
const btnGhost = { border: '1px solid rgba(255,255,255,0.5)', color: '#fff' } as const
...
export default function TeacherHub({ name, subjects, classes, assignments, resources }: { name: string; subjects: string[]; classes: HubClass[]; assignments: HubAssignment[]; resources: HubResource[] }) {
  ...
  return (
    <>
      <div ref={ref} style={{ height: `calc(100vh - ${NAV_H}px)`, overflowY: 'scroll', scrollSnapType: 'y mandatory', scrollBehavior: 'smooth', background: '#0a0a14' }}>
        {/* ── 1. Classes ── */}
        <section className={pad} style={{ ...sectionStyle(0), overflowY: 'auto' }}>
          <div className="mx-auto max-w-6xl py-12 text-white">
            <div className={eyebrow} style={{ color: 'rgba(255,255,255,0.4)' }}>TEACHER DASHBOARD</div>
            ...
            <div className="mt-6 max-w-2xl rounded-2xl p-4" style={glass}>
              <div className="mb-2 text-xs font-black tracking-widest" style={{ color: 'rgba(255,255,255,0.55)' }}>SUBJECTS YOU TEACH</div>
              <SubjectPicker initial={subjects} dark onChange={setMine} />
            </div>
            ...
            <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_340px]">
              <div>
                ...
                <div className="mb-4 rounded-2xl p-4" style={glass}><CreateClassForm dark /></div>
                {classes.length === 0 ? (
                  <p className="text-sm" style={{ color: 'rgba(255,255,255,0.55)' }}>No classes yet — create one above and share its code with your students.</p>
                ) : (
                    ...
                    {classes.map((c, i) => (
                      <div key={c.id} className="overflow-hidden rounded-2xl" style={glass}>
                        ...
                        <div className="p-4">
                          ...
                          <div className="mt-1 text-xs font-bold tracking-widest" style={{ color: 'rgba(255,255,255,0.5)' }}>CODE <span className="text-white">{c.join_code}</span></div>
                          <div className="mt-2 text-sm" style={{ color: 'rgba(255,255,255,0.65)' }}>{c.students} student{c.students === 1 ? '' : 's'} · {c.assignments} assignment{c.assignments === 1 ? '' : 's'}</div>
              ...
              <aside className="rounded-2xl p-5 self-start" style={glass}>
                ...
                {assignments.length === 0 ? (
                  <p className="mt-3 text-sm" style={{ color: 'rgba(255,255,255,0.55)' }}>No ongoing or recent assignments.</p>
                ) : (
                  assignments.map((a) => (
                    <Link key={a.id} href={`/classes/${a.classId}?tab=insights`} className="mt-3 block rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.06)' }}>
                      <div className="text-sm font-bold">{a.title}</div>
                      <div className="text-xs" style={{ color: 'rgba(255,255,255,0.55)' }}>{a.className} · {a.kind === 'paper' ? 'Past paper' : 'Topic revision'}</div>
                      <div className="mt-2 h-1.5 overflow-hidden rounded-full" style={{ background: 'rgba(255,255,255,0.15)' }}>
                        <div className="h-full rounded-full" style={{ width: `${a.total ? Math.round((a.done / a.total) * 100) : 0}%`, background: '#fff' }} />
                      </div>
                      <div className="mt-1 text-[11px]" style={{ color: 'rgba(255,255,255,0.55)' }}>{a.done} of {a.total} done</div>
                    </Link>
                  ))
                )}
              </aside>
            </div>
            <div className="mt-10 text-center text-[11px] font-black tracking-[0.25em]" style={{ color: 'rgba(255,255,255,0.3)' }}>SCROLL FOR FREE RESOURCES ↓</div>
          </div>
        </section>
        ...
        <section className={pad} style={{ ...sectionStyle(1), overflowY: 'auto' }}>
          <div className="mx-auto max-w-6xl py-12 text-white">
            <div className={eyebrow} style={{ color: 'rgba(255,255,255,0.4)' }}>FREE · FOR YOUR SUBJECTS</div>
            ...
            <p className="mt-4 max-w-xl text-base" style={{ color: 'rgba(255,255,255,0.6)', fontFamily: 'Georgia, serif' }}>
            ...
            {shown.length === 0 ? (
              <p className="mt-8 text-sm" style={{ color: 'rgba(255,255,255,0.7)' }}>Choose your subjects at the top of your dashboard to see resources here.</p>
            ) : (
                ...
                {shown.map((r) => (
                  <div key={r.slug} className="rounded-2xl p-5" style={glass}>
                    ...
                    {firstClass && (r.papers > 0 || r.topics > 0) ? (
                      ...
                      <div className="mt-4 text-xs" style={{ color: 'rgba(255,255,255,0.55)' }}>Create a class first to assign this.</div>
                    ) : (
        ...
        <section className={pad} style={{ ...sectionStyle(2), overflowY: 'auto' }}>
          <div className="mx-auto max-w-6xl py-12 text-white">
            <div className={eyebrow} style={{ color: 'rgba(255,255,255,0.4)' }}>LIVE · HOST FROM ANY DEVICE · STUDENTS JOIN WITH A CODE</div>
              ...
              return (
                <div key={year} className="mt-8">
                  ...
                  <div className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {acts.map((a) => (
                      <Link key={a.id} href={`/design/live/${a.id}`} className="rounded-2xl p-5 transition-transform hover:-translate-y-1" style={{ background: `linear-gradient(145deg, ${a.theme.from}, ${a.theme.via} 55%, ${a.theme.to})`, border: '1px solid rgba(255,255,255,0.16)' }}>
        ...
        {GATES.map((g, i) => {
          ...
          return (
            <div key={g.id} className="flex cursor-pointer items-center gap-2" onClick={() => goTo(i)} onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(null)}>
              <div style={{ width: on ? 12 : hov ? 10 : 8, height: on ? 12 : hov ? 10 : 8, transform: 'rotate(45deg)', background: on ? '#fff' : hov ? 'rgba(255,255,255,0.5)' : 'rgba(255,255,255,0.2)', border: on ? 'none' : '1px solid rgba(255,255,255,0.35)', transition: 'all 0.2s', boxShadow: on ? '0 0 10px #fff' : 'none' }} />
              <span style={{ color: on ? 'rgba(255,255,255,0.95)' : hov ? 'rgba(255,255,255,0.88)' : 'rgba(255,255,255,0.28)', fontSize: on || hov ? 11 : 9.5, fontWeight: on ? 900 : 700, letterSpacing: '0.15em', whiteSpace: 'nowrap', userSelect: 'none' }}>{g.label}</span>
            </div>
          )
        })}
      </nav>
      <div className="fixed z-50 hidden text-xs font-black tracking-widest md:block" style={{ right: '2rem', bottom: '2rem', color: 'rgba(255,255,255,0.25)' }}>
...
function sectionStyle(i: number) {
  ...
  return {
    ...
    background: `linear-gradient(145deg, ${g.from} 0%, ${g.via} 55%, ${g.to} 100%)`,
  }
}
