...
interface PlayerRow { session_code: string; user_id: string; points: number }
...
export default async function ClassPage({ params }: { params: { id: string } }) {
  ...
  const [{ data: players }, { data: grades }] = codes.length
    ? await Promise.all([
        supabase.from('live_players').select('id, session_code, user_id, points').in('session_code', codes),
  ...
  function cell(userId: string, code: string): { text: string; tone: string } {
    ...
    return { text: `Joined · ${p.points} pts`, tone: 'var(--text)' }
  }
